<template>
  <div class="navbar">
    <div class="find-input">
      <input type="text" placeholder="Search natives" v-model="searchStr" />
      <i
        v-show="searchStr.length > 0"
        class="icon-cancel"
        @click="searchStr = ''"
      />
    </div>

    <div class="natives-list-out">
      <div v-if="searchStr.length === 0" class="natives-list">
        <nav-collapse v-for="cat in categories" :key="cat" :category="cat" />
      </div>

      <div v-else class="natives-list">
        <native-list-item
          v-for="n in searchResult"
          :key="n.hash"
          :name="n.altName"
          :quality="n.quality"
          :hash="n.hash"
        />
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { ref, onMounted, watch, computed } from "vue";
import { useRouter } from "vue-router";
import { useStore } from "vuex";
import NavCollapse from "@/components/NavCollapse.vue";
import NativeListItem from "@/components/NativeListItem.vue";

const router = useRouter();
const store = useStore();

const searchStr = ref(router.currentRoute.value.query?.query ?? "");
const searchResult = ref<unknown[]>([]);
let searchTimeout: number | null = null;

const categories = computed(() => {
  return Object.keys(store.state.nativesByCat);
});

onMounted(() => {
  searchStr.value = router.currentRoute.value.query?.query ?? "";
});

watch(searchStr, async (str: string) => {
  if (str.length > 0) {
    if (searchTimeout) {
      clearTimeout(searchTimeout);
    }

    searchTimeout = setTimeout(async () => {
      searchTimeout = null;
      searchResult.value = await store.dispatch("search", str);
    }, 100);
  } else {
    searchResult.value = [];
  }
});
</script>

<script lang="ts">
export default {
  name: "AppNavigation",
  components: {
    NavCollapse,
    NativeListItem,
  },
};
</script>

<style lang="scss" scoped>
.navbar {
  width: 300px;
  height: 100%;
  background: rgba(196, 196, 196, 0.2);
  display: flex;
  flex-direction: column;
}

.find-input {
  position: relative;
  margin: 20px 20px 15px 20px;

  input {
    &:focus {
      outline: none;
    }

    &::placeholder {
      color: #969696;
      user-select: none;
    }

    color: white;
    padding-left: 20px;
    box-sizing: border-box;
    width: 100%;
    height: 100%;
    border-radius: 10px;
    background: rgba(0, 0, 0, 0.4);
    border: 0px solid black;
    height: 35px;
  }

  .icon-cancel {
    position: absolute;
    padding: 8.5px;
    top: 0;
    right: 0;
    opacity: 0.5;
    cursor: pointer;

    &:hover {
      opacity: 0.6;
    }
  }
}

.natives-list-out {
  display: block;
  position: relative;
  flex: 1 1;
}

.natives-list {
  position: absolute;
  width: 100%;
  height: 100%;
  display: flex;
  flex-direction: column;
  box-sizing: border-box;
  padding: 0px 20px 0px 20px;
  overflow-x: hidden;
  overflow-y: auto;
  scrollbar-width: thin;
  scrollbar-color: #aaa transparent;
}

.natives-list::-webkit-scrollbar {
  width: 5px;
}

.natives-list::-webkit-scrollbar-thumb {
  background-color: #aaa;
  min-height: 40px;
}

.footer {
  height: 40px;
}
</style>
